...
const sortStarWarsCharacters = (starWarsArr) => {
  // Solution code here...
  let sorted = starWarsArr.sort((a,b) => {
...
recipe, below. Rather than taking the entire recipe, you only want a list of the
item names.

Write a function named listFoods that takes in the recipe and returns a new array
of the food items without any amount or units. Just the name. For example,
 '1 cup flour' will return 'flour'.

Use slice for this function, maybe more than once. The Array.indexOf() method may
 also be helpful.
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(element => {
    ...
    let food = element.slice(index + 1, element.length)
    result.push(food)
  })
  return result;
};
