...
uses Regex to create a new array of only those strings that match the following
rules:
...
returns an array of strings with the first character in upper case and the
rest as is.

For example, ['apple', 'banana', 'MacGyver'] returns ['Apple', 'Banana',
'MacGyver'].
------------------------------------------------------------------------------------------------ */
...
Write a function named biggerThanLuke that, given the Star Wars data,
 below, returns the names of the characters whose mass is greater than Luke's.

The names should be combined into a single string with each character name
 separated by a dash.
...
let biggerThanLuke = (arr) => {
  ...
  if ((arr.length === 0)) {
    ...
    let mass = parseInt(arr[0].mass || 0);
    for (let x = 1; x < arr.length; x++) {
      if (parseInt(arr[x].mass) > mass) {
        // console.log(arr[x]);
        finalArr.push(arr[x].name);
      }
    }
...
each of which has a particular property, and sorts those objects
 by that property, lowest to highest, returning the same array.
...
Write a function that determines if a given URL is secure, beginning
with https://
...
Write a function named detectTicTacToeWin that accepts a two-dimensional array
 of strings. Each string is guaranteed to be either "X", "O" or an empty string.
  Your function should check to see if any row, column, or either diagonal
  direction has three matching "X" or "O" symbols (non-empty strings),
  three-in-a-line.

This function should return either true or false to indicate if someone won
the game.
...
const detectTicTacToeWin = (board) => {
  // Helper function to check if three coordinates have the same non-empty value
  const helpCheck = (row1, col1, row2, col2, row3, col3) => {
    const set1 = board[row1][col1];
    const set2 = board[row2][col2];
    const set3 = board[row3][col3];
    return set1 !== '' && set1 === set2 && set2 === set3;
  };
  // Check rows
